...
CBalloonTip* CBalloonTip::Show(CPoint pt, CSize size, CString strMessage, LOGFONT lf, UINT nSecs, BOOL bBalloonUp)
{
    ...
    if (bBalloonUp)
    {
	    ...
	    nRectTop    = (UINT)(pt.y - size.cy);
	    nRectBottom = (UINT)(pt.y);
    }
    else
    {
...
void CBalloonTip::MakeVisisble(UINT nSecs)
{
    ...
    SetTimer(ID_TIMER, (nSecs * 10), NULL);
...
int CBalloonTip::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
    ...
    if (m_bBalloonUp)
    {
        ...
        m_rectText.left   = (long)(t_Rect.Width() * 0.10); 
        m_rectText.right  = (long)(t_Rect.Width() * 0.90);
        m_rectText.top    = (long)(t_Rect.Height() * 0.10); 
        m_rectText.bottom = (long)(t_Rect.Height()* 0.70); 
	    ...
	    m_rgnRoundRect.CreateRoundRectRgn(
            ...
            t_Rect.top, 
            t_Rect.right,
            (int)(t_Rect.bottom - (t_Rect.Height() * 0.25)), 
            (int)(abs(t_Rect.left - t_Rect.right) * 0.18),
            (int)(t_Rect.Height() * 0.25)
        );
   
        // Start the process of creating the balloon tip
	    CPoint ptTri[4];
	    ptTri[0].x = (LONG)((t_Rect.left) + (t_Rect.Width() * 0.93));
	    ptTri[0].y = (LONG)((t_Rect.bottom) - (t_Rect.Height() * 0.40));
    
	    ptTri[1].x = (LONG)((t_Rect.left) + (t_Rect.Width() * 0.65));
	    ptTri[1].y = t_Rect.bottom;

	    ptTri[2].x = (LONG)((t_Rect.left) + (t_Rect.Width() * 0.70));
	    ptTri[2].y = (LONG)((t_Rect.bottom) - (t_Rect.Height() * 0.40));
   
	    ptTri[3].x = (LONG)((t_Rect.left) + (t_Rect.Width() * 0.93));
	    ptTri[3].y = (LONG)((t_Rect.bottom) - (t_Rect.Height() * 0.40));
    ...
    else
    {
        ...
        m_rectText.left   = (LONG)(t_Rect.Width() * 0.10);
        m_rectText.right  = (LONG)(t_Rect.Width() * 0.90);
        m_rectText.top    = (LONG)(t_Rect.Height() * 0.30);
        m_rectText.bottom = (LONG)(t_Rect.Height() * 0.90);
        
        // Create an elliptical region out of the client rectangle
	    m_rgnRoundRect.CreateRoundRectRgn(
            t_Rect.left,
            (int)(t_Rect.bottom - (t_Rect.Height() * 0.75)), 
            t_Rect.right, 
            t_Rect.bottom, 
            (int)(abs(t_Rect.left - t_Rect.right) * 0.18),
            (int)(t_Rect.Height() * 0.25)
        ...
        // Start the process of creating the balloon tip
	    CPoint ptTri[4];
	    ptTri[0].x = (LONG)((t_Rect.left) + (t_Rect.Width() * 0.07));
	    ptTri[0].y = (LONG)((t_Rect.Height() * 0.40));
            
	    ptTri[1].x = (LONG)((t_Rect.left) + (t_Rect.Width() * 0.35));
	    ptTri[1].y = (LONG)(t_Rect.bottom - t_Rect.Height());

	    ptTri[2].x = (LONG)((t_Rect.left) + (t_Rect.Width() * 0.30));
	    ptTri[2].y = (LONG)(t_Rect.Height() * 0.40);
           
	    ptTri[3].x = (LONG)((t_Rect.left) + (t_Rect.Width() * 0.07));
	    ptTri[3].y = (LONG)(t_Rect.Height() * 0.40);
    ...
    if (iRetComb == ERROR)
	{
        ::AfxMessageBox(_T("ERROR in Combining Region"));
		return -1;
	}
...
void CBalloonTip::OnPaint() 
{
    ...
    CBrush brFillBrush;
    brFillBrush.CreateSolidBrush(RGB(100, 100, 50)); // light yellow
